...
compilation_unit: 	package_statement import_statement class_stmt     {$$ = $3;}
             		| T_IDENTIFIER        { $$ = createnode(0,0, "node"); }
             		;
...
char_e: 	id   						{$$ = $1;}
			...
			| T_CONSTANT                {$$ = createnode(0,0, yytext);}
			;
...
else_if_blocks:	T_ELSE else_if_block									{$$ = $2;}  
           		| else_if_blocks  T_ELSE else_if_block					{$$ = createnode($1,$3, "ELSE_IF_BLOCKS");}
           		;

else_if_block: 	T_IF T_OC rel_e T_CC T_OF stmnt T_CF  					{$$ = createnode($3,$6, "ELSE_IF");}
      			| T_OF stmnt T_CF										{$$ = createnode(0,$2, "ELSE");}
          		;
